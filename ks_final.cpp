...
//	Employee NV[N];
	...
	void GhiThongKe(char stat[], char s[], char name[], int y);
...
void Motel::Pay(int from)
{
	...
	if( Room[i].Status == true )
	{
		if( Room[i].Bill == 0 )
		{
			...
			GhiThongKe(stat,"Tien phong",Room[i].Cus.Name, Room[i].Price);
		}
		else
		{
			Revenue += Room[i].Bill;
			...
			GhiThongKe(stat,"Tien phong",Room[i].Cus.Name, Room[i].Bill);
		}
...
void Motel::Fired(char *empid)
{
	...
	if(strcmp(First->NV.ID,empid)==0)
	{
		First=First->next;
		First->prev=NULL;
		return;
	}
	else if(strcmp(Last->NV.ID,empid)==0)
	{
		Last=Last->prev;
		Last->next=NULL;
...
//		continue;
		...
		printf("\n|%15s", p->NV.ID);
		printf("|%-20s", p->NV.Name);
		printf("|%8d", p->NV.Year);
		printf("|%20s", p->NV.Home);
		printf("|%15d",p->NV.Salary);
		switch(p->NV.Job)
		{
			case 1:
				printf("|            Tiep tan|");
...
//	p=First;
	while(p!=NULL)
	{
		cout<<"Nhan Vien: "<<p->NV.Name<<" co luong tinh tu luc bat dau " <<p->NV.Income<<endl;
		Revenue -= p->NV.Income;
		GhiThongKe(stat,"Tien luong NV",p->NV.Name, -p->NV.Income);
		p->NV.Income = 0;
		p=p->next;
	}
	...
	if(Room[i].Status==true)
	{
		...
		switch(a)
		{
			case 1:
				...
				GhiThongKe(stat,"don dep",Room[i].Cus.Name,50000);
			...
			case 2:
				...
				GhiThongKe(stat,"giat ui",Room[i].Cus.Name,70000);
			...
			case 3:
				...
				Room[i].Bill += 50000;
				GhiThongKe(stat,"Don ly chen",Room[i].Cus.Name,50000);
				//
			...
			case 4:
				...
				GhiThongKe(stat,"do an nhanh",Room[i].Cus.Name,1000000);
			...
			case 5:
				...
				GhiThongKe(stat,"thuoc la",Room[i].Cus.Name,20000);
			...
			case 7:
				{
					...
					switch(b){
						case 1:
							...
							GhiThongKe(stat,"thue xe may",Room[i].Cus.Name,180000);
							break;
						case 2:
							...
							GhiThongKe(stat,"thue xe oto",Room[i].Cus.Name,1200000);
							break;
						default:
...
void Motel::Maintain(int i)
{
	...
	switch( Room[i].Fixing )
	{
		case 1:
			...
			Revenue -= 500000;
			GhiThongKe(stat,"Bao Tri","Khach San", -500000);
			break;
		case 2:
			printf("Da sua chua nhung khu vuc am thap.");
			Revenue -= 450000;
			GhiThongKe(stat,"Bao Tri","Khach San", -450000);
			break;
		case 3:
			printf("Da gia co tuong va cac vat dung cach am.");
			Revenue -= 400000;
			GhiThongKe(stat,"Bao Tri","Khach San", -400000);
			break;
		case 4:
			printf("Da sua lai giuong.");
			Revenue -= 350000;
			GhiThongKe(stat,"Bao Tri","Khach San", -350000);
			break;
		case 5:
			printf("Da sua chua he thong nuoc.");
			Revenue -= 300000;
			GhiThongKe(stat,"Bao Tri","Khach San", -300000);
			break;
		case 0:
...
void Motel::DocThongKe(char stat[])
{
	...
	printLine(78);
	cout<<"|Doi tuong su dung   |Dich vu             |Gia tien |Thoi gian               |";
	cout<<"\n------------------------------------------------------------------------------";
	while (fscanf(fp, "%[^\t] %[^\t] %d %[^\n]\n", &name, &dichvu,&price,&time) != EOF)
    {
        //cout<<"1"<<endl;
		printf("\n|%-20s|%-20s|%9d|%s|", name, dichvu, price,time );
    }
	printLine(78);
	fclose (fp);
}
...
void Motel::GhiFileNV(char fileNameNV[]) 
{
	...
	while(p!=NULL)
	{
		fprintf(fp, "%-s\t %-s\t %4d %-s\t %d %d\n", p->NV.Name, p->NV.ID, p->NV.Year, p->NV.Home, p->NV.Job, p->NV.Salary );
		p=p->next;
	}
...
void Motel::GhiThongKe(char stat[], char s[], char name[], int y)
{
	FILE * fp;
	fp = fopen(stat, "a");
	...
	fprintf(fp, "%s\t %s\t %d %s",name,s,y,ctime(&curtime));
